// Headless smoke test: Midnight Market (market) — a lantern-lit medium-sized
// night bazaar. Geometry, votability, tier reachability, spawn footing,
...
{
  const m = MAPS.market;
  check('market exists and is named Midnight Market', !!m && m.name === 'Midnight Market');
  check('market is votable (not hidden)', MAP_IDS.includes('market'));
  ...
  check('all four spawns stand on the plaza', m.spawns.length === 4
...
// Both stall roofs must be single-jumpable from the plaza; the banner walk
// is allowed to need a double jump straight off the floor.
{
  ...
  check('both stall roofs are single-jumpable from the plaza', statics.every(p => reach(p, 155)));
  const banner = m.plats.find(p => p.move);
  check('the banner walk needs a double jump off the floor, not a single', !reach(banner, 155) && reach(banner, 259));
}

// --- 3. deterministic geometry (one live mover) ---
{
  const at = t => platsAt('market', t);
  check('platsAt is deterministic', JSON.stringify(at(500)) === JSON.stringify(at(500)));
  check('the banner walk actually drifts over time', JSON.stringify(at(0)) !== JSON.stringify(at(4)));
...
{
  const g = new Game(players, 9, 'market');
  for (let i = 0; i < 30; i++) g.step();
  check('both fighters land grounded on the plaza', g.fighters.every(f => f.grounded && Math.abs(f.y + 32) < 1));
  const inp = blankInput();
  inp.mx = 1;
  g.setInput('A', inp);
  for (let i = 0; i < 300; i++) g.step();
  check('5s sim runs clean', !g.over && g.fighters.every(f => !f.dead));
  check('runner is still inside the blast box', g.fighters[0].x < MAPS.market.blast.r);
}
...
// --- 5. blast KOs at the far bounds ---
{
  ...
  f.x = MAPS.market.blast.l - 10;
  g.step();
  check('crossing the west blast line costs a stock', f.stocks === before - 1);
  const g2 = new Game(players, 9, 'market');
  const f2 = g2.fighters[0];
  f2.x = -190; f2.y = MAPS.market.blast.t - 10;
  g2.step();
...
// --- 6. snapshot fidelity ---
{
  ...
  check('replica geometry matches', JSON.stringify(platsAt('market', snap.tk)) === JSON.stringify(platsAt('market', g.tick)));
}

// --- 7. other maps untouched ---
{
  check('temple keeps its seven plats', MAPS.temple.plats.length === 7);
  check('canyon keeps its rope bridge', MAPS.canyon.plats.filter(p => p.move).length === 1);
