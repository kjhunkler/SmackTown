...
const THEMES = {
  ...
  shrine: {
    sky: ['#2c1030', '#5c2a4a', '#e88aa0'],
    motif: 'duskmoth',
    stars: 0.4,
    ambient: 'cloudwisp',
    deck: '#4a2e34', lip: '#6e4650', trim: '#ff9ec0',
    plat: '#5c3a42', platTop: '#84545e',
  },
  galleon: {
    sky: ['#081020', '#122438', '#1c3a4a'],
    motif: 'moon',
    stars: 0.6,
    ambient: 'cloudwisp',
    deck: '#4a3220', lip: '#6a4a30', trim: '#ffd06a',
    plat: '#5c3e28', platTop: '#7e5838',
  },
  observatory: {
    sky: ['#0a0a24', '#16123c', '#241a4a'],
    motif: 'ringworld',
    stars: 1.0,
    deck: '#262a4a', lip: '#3e4472', trim: '#b48aff',
    plat: '#323764', platTop: '#4d5490',
  },
  citadel: {
    sky: ['#0b0d1c', '#1c2140', '#2c3050'],
    motif: 'moon',
    stars: 0.3,
    ambient: 'rain',
    deck: '#2e3248', lip: '#4a4f6e', trim: '#9fd0ff',
  ...
  roots: {
    ...
    stars: 0.5,
    ambient: 'fireflies',
    deck: '#3a2e22', lip: '#57462f', trim: '#8fe37a',
    plat: '#4a3a28', platTop: '#6a5438',
  },
  training: {
...
export class Renderer {
    ...
    for (const ev of events || []) {
      ...
      switch (ev.e) {
        ...
        case 'crush':
          ...
          break;
        case 'burstboom': {
          // a shell detonating: violet for magic bursts, fiery for fireballs
          const ring = ev.fire ? '#ff8a2e' : '#c9a7ff';
          const spark = ev.fire ? '#ffb35c' : '#b98cff';
          this.rings.push({ x: ev.x, y: ev.y, r0: 12, r1: ev.r || 90, t: 0, life: 0.28, color: ring, w: 5 });
  ...
  _fighter(ctx, f, isMe, t) {
    ...
    // neutral-air spin: whirl continuously through the whole move — purely
    // a canvas rotation, so the sim hurtbox underneath is untouched, same
    // as every other rotate effect here
    const spinning = f.atk === 'nspin' && f.state === 'attack';
    if (spinning) ctx.rotate(t * 20 * (f.facing || 1));
    ...
    // The spin also tucks into a tight crouch for its whole duration.
    const sq = this.squash.get(f.id);
    const squish = sq ? 1 - sq.a * (sq.t / LAND_SQUASH_T) : 1;
    const stretch = clamp(1 + Math.abs(f.vy) / 3500, 1, 1.25) * squish * (spinning ? 0.7 : 1);
