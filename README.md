...
| Fighter workshop | `js/ui.js` | The shop UI — stats (Power/Speed/Defense/Agility), weapons (Bare Fists, Sword, Magic), abilities (Fireball, Dash Strike, Shockwave, Uppercut, Counter, Blink) and augments (Vampiric, Thorns, Featherweight, Heavyweight, Berserker, Second Wind). |
| Weapons | `js/game.js` | The strong attack is the equipped weapon. **Bare fists** (free, and a real choice) keep the classic smash kit; the **sword** lunge-slashes in any aim direction — huge damage, light launch, and a much faster charge; **magic** casts knockback bursts whose range/power scale hard with charge time, fueled by a self-recharging mana pool. Neutral strong attacks don't exist — a neutral swipe is the side strike the way you face. |
